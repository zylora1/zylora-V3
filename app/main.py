...
from .super_admin_assistant import router as super_admin_assistant_router
...
def platform_blog_post(post_slug:str):
    ...
    schema_json=json.dumps(schema,ensure_ascii=False,separators=(',',':')).replace('</','<\\/')
    # Content is sanitized again at the rendering boundary so older drafts are
    # safe even if they predate the write-time sanitizer.
    paras=f'<article class="z-blog-rich-content">{sanitize_rich_html(str(post["content"]))}</article>'
