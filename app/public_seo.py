...
PRODUCTS: dict[str, dict[str, Any]] = {
    ...
    "/website-builder": {
        ...
        "capabilities": ["AI-assisted site creation without a locked starting template", "Direct manipulation editing for type, buttons, cards, images and layout", "Built-in forms, AI Sales Assistant and appointment flows", "SEO settings, sitemap, robots and hosted publishing"],
    ...
    "/zylora-studio": {
        ...
        "faq": [("Can I edit responsive layouts?", "Studio includes responsive preview modes so you can review the site across the supported viewport states before publishing."), ("Can I publish from Studio?", "Yes. Publishing remains part of the existing Zylora workflow after review and site settings are complete."), ("Where is the published site hosted?", "Zylora hosts the published site. You can use a Zylora address or connect a domain you own." )],
...
def _info_page(path: str, item: dict[str, Any]) -> HTMLResponse:
    ...
    return HTMLResponse(_head(item['title'], item['description'], canonical, schema) + '<body>' + body.replace('✦ ', '') + '</body></html>', headers={'Cache-Control': 'public,max-age=300'})
...
def _help_index() -> HTMLResponse:
    ...
    return HTMLResponse(_head('Zylora Help Center | Practical product guidance', 'Find practical guidance for Zylora accounts, Studio editing, publishing, billing and customer workflows.', canonical, schema) + '<body>' + body.replace('✦ ', '') + '</body></html>', headers={'Cache-Control': 'public,max-age=300'})
...
def _help_article(slug: str) -> HTMLResponse:
    ...
    return HTMLResponse(_head(item['title'] + ' | Zylora Help', item['description'], canonical, schema) + '<body>' + body.replace('✦ ', '') + '</body></html>', headers={'Cache-Control': 'public,max-age=300'})
...
def template_detail(slug: str) -> HTMLResponse:
    ...
    return HTMLResponse(_head(f'{name} Website Template | Zylora', description, canonical, schema) + '<body>' + body.replace('✦ ', '') + '</body></html>', headers={'Cache-Control': 'public,max-age=300'})
