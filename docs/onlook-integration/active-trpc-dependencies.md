...
Static call-site inventory is complete. A browser-backed runtime trace is not
yet available because the canonical E2E runner requires PostgreSQL and no
PostgreSQL service is running in this environment. Therefore the table below
is a source-level classification, not a claim that every call has executed.

The latest repository suite recorded 527 passed, 1 skipped, and 59 warnings in
495.07 seconds.
The cross-browser fixture now reaches all three browser engines, but it does
not enable the authenticated runtime trace; no method below has been promoted
on the basis of fixture mount evidence alone.

## Active call classification

| Method | Call sites | Classification | Current handling |
|---|---|---|---|
| `project.get` | breadcrumb, publish domain | EDITOR_REQUIRED | Current Zylora project context from `ZyloraProjectAdapter` |
...
| `project.create`, `project.fork` | create/clone hooks | ZYLORA_PLATFORM_REQUIRED | Explicit unsupported error; project creation remains Zylora dashboard-owned |
...
| `user.settings.get/upsert` | chat controls | OPTIONAL | Explicit Zylora defaults/success compatibility state; no editor source mutation |
...
| `chat.message.getAll/updateCheckpoints` | chat panel / chat hook | OPTIONAL | Explicit local compatibility state; runtime trace still pending |
...
| `useUtils` | invitation/chat controls | OPTIONAL | Explicit finite invalidation hooks; no generic chained proxy |
...
The command currently stops at environment startup when PostgreSQL is absent;
that is an external runtime blocker, not a passing E2E result.
