...
def main() -> None:
    ...
    with TestClient(app) as client, sync_playwright() as p:
        ...
        hosted_resp = client.get(public_url)
        check(hosted_resp.status_code == 200, 'published website remains hosted by Zylora after dashboard navigation')
        ...
        page.locator('.rail-btn[data-view="billing"]').click()
        check(client.get('/api/billing').json()['plan'] == 'FREE', 'AI-site publish does not silently change the subscription plan')
