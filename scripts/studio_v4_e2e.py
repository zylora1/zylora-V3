...
def main():
    ...
    try:
      with sync_playwright() as pw:
        ...
        page.locator('.studio-sidebar-left header button[aria-label="Close tool panel"]').click();image.scroll_into_view_if_needed();page.get_by_label('Crop').click();page.wait_for_selector('.crop-toolbar',timeout=5000);crop_frame=image.locator('.studio-image-frame');crop_box=crop_frame.bounding_box();assert crop_box;cx=crop_box['x']+crop_box['width']/2;cy=crop_box['y']+crop_box['height']/2;print('CROP POINT DEBUG',crop_box,page.evaluate("([x,y])=>{const e=document.elementFromPoint(x,y);return {tag:e?.tagName,className:e?.className}}",[cx,cy]),flush=True);page.mouse.move(cx,cy);page.mouse.down();page.mouse.move(cx+28,cy+12,steps=4);page.mouse.up();page.wait_for_timeout(150);print('CROP DRAG DEBUG',crop_frame.locator('img').get_attribute('style'),flush=True);page.get_by_label('Zoom in crop').evaluate("el=>el.click()");page.wait_for_timeout(150);print('CROP UI DEBUG',page.get_by_label('Crop zoom').input_value(),crop_frame.locator('img').get_attribute('style'),flush=True);page.get_by_text('Done',exact=True).evaluate("el=>el.click()");check(page.locator('.crop-toolbar').count()==0,'image crop mode supports drag, zoom and Done',checks)
        ...
        page.wait_for_function("document.querySelector('.save-state')?.textContent.includes('Saved')",timeout=20000);saved=client.post(f'/api/sites/{site_id}/studio-migrate',headers=headers).json()['document'];nodes=saved['pages']['home']['nodes'];saved_heading=next(n for n in nodes.values() if n.get('content',{}).get('text')=='Browser edited production heading');saved_image=nodes[image_id];crop=saved_image.get('content',{}).get('crop',{});print('CROP DEBUG',crop,saved_image.get('style',{}).get('css',{}),flush=True);check(saved_heading['style']['css'].get('fontSize')=='42px' and saved_heading['content'].get('href')=='https://example.com/' and any(i.get('trigger')=='hover' and i.get('effect')=='lift' for i in saved_heading.get('interactions',[])) and any(i.get('trigger')=='animation' and i.get('effect')=='fade' for i in saved_heading.get('interactions',[])) and crop.get('scale',1)>1 and saved_image['style']['css'].get('objectFit')=='contain','autosave persists typography, link, image crop/fitting, and effects',checks)
