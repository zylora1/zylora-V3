...
def main():
    ...
    try:
      with sync_playwright() as pw:
        ...
        page.get_by_title('Add Heading').click();heading=page.locator('[data-studio-type="heading"]').last;heading.click();editor=heading.locator('.studio-text-editor');editor.fill('Browser edited production heading');editor.press('Tab')
        ...
        page.locator('.tool-rail button[title="Assets"]').click();page.locator('.asset-drop input').set_input_files(image_path);page.wait_for_function("document.querySelector('.panel-message')?.textContent.includes('Upload complete')",timeout=15000);page.locator('.asset-grid button').first.click();page.wait_for_timeout(250)
