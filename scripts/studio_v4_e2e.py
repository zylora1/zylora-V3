...
def main():
    ...
    try:
      with sync_playwright() as pw:
        ...
        check(page.locator('.tool-rail button').count()==8,'Studio exposes the eight current workspaces',checks)
        ...
        image_to_clear=page.locator('[data-studio-type="image"]').last; page.get_by_text('More',exact=True).click(); page.locator('.context-more-popover').get_by_role('button', name='Send to back').click(); page.wait_for_timeout(120)
        heading_id=heading.get_attribute('data-studio-id'); heading.evaluate('el=>el.click()'); page.wait_for_timeout(120); check(page.locator('.selection-toolbar').count()==1,'selection opens contextual top toolbar',checks)
        font_size=page.get_by_label('Font size');font_size.fill('42');font_size.press('Enter');page.get_by_label('Font family').click();page.get_by_role('button',name=re.compile('Georgia')).click();page.get_by_label('Text color').fill('#123456');page.get_by_text('More',exact=True).click();page.get_by_label('Hover effect').select_option('lift');page.get_by_label('Animation').select_option('fade');page.get_by_text('More',exact=True).click()
        ...
        page.wait_for_function("document.querySelector('.save-status-control')?.getAttribute('title')==='Saved'",timeout=20000);saved=client.post(f'/api/sites/{site_id}/studio-migrate',headers=headers).json()['document'];nodes=saved['pages']['home']['nodes'];saved_heading=next(n for n in nodes.values() if n.get('content',{}).get('text')=='Browser edited production heading');saved_image=nodes[image_id];crop=saved_image.get('content',{}).get('crop',{});saved_css=saved_heading['style']['css'];check(saved_css.get('fontSize')=='42px' and saved_css.get('fontFamily')=='Georgia' and saved_css.get('color')=='#123456' and float(saved_css.get('opacity',1))==.65 and bool(saved_css.get('width')) and bool(saved_css.get('left')) and bool(saved_css.get('top')) and saved_heading['content'].get('href')=='https://example.com/' and any(i.get('trigger')=='hover' and i.get('effect')=='lift' for i in saved_heading.get('interactions',[])) and any(i.get('trigger')=='animation' and i.get('effect')=='fade' for i in saved_heading.get('interactions',[])) and crop.get('scale',1)>1 and saved_image['style']['css'].get('objectFit')=='contain','autosave persists geometry, typography, opacity, link, image crop/fitting, and effects',checks)
