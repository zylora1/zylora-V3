...
def main():
    ...
    try:
      with sync_playwright() as pw:
        ...
        check(page.locator('.tool-rail button').count()==7,'Studio exposes the seven current workspaces',checks)
        ...
        page.wait_for_function("document.querySelector('.cloud-save')?.getAttribute('title')==='Saved'",timeout=20000);saved=client.post(f'/api/sites/{site_id}/studio-migrate',headers=headers).json()['document'];nodes=saved['pages']['home']['nodes'];saved_heading=next(n for n in nodes.values() if n.get('content',{}).get('text')=='Browser edited production heading');saved_image=nodes[image_id];crop=saved_image.get('content',{}).get('crop',{});saved_css=saved_heading['style']['css'];check(saved_css.get('fontSize')=='42px' and saved_css.get('fontFamily')=='Georgia' and saved_css.get('color')=='#123456' and float(saved_css.get('opacity',1))==.65 and bool(saved_css.get('width')) and bool(saved_css.get('left')) and bool(saved_css.get('top')) and saved_heading['content'].get('href')=='https://example.com/' and any(i.get('trigger')=='hover' and i.get('effect')=='lift' for i in saved_heading.get('interactions',[])) and any(i.get('trigger')=='animation' and i.get('effect')=='fade' for i in saved_heading.get('interactions',[])) and crop.get('scale',1)>1 and saved_image['style']['css'].get('objectFit')=='contain','autosave persists geometry, typography, opacity, link, image crop/fitting, and effects',checks)
