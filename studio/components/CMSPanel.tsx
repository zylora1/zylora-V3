...
export function CMSPanel({contentOnly=false}:{contentOnly?:boolean}) {
  ...
  return <div className="cms-panel">
    ...
    {!contentOnly&&tab==='bindings'&&<><p>{selectedNode?`Selected: ${selectedNode.type} · ${selectedNode.id}`:'Select one canvas node.'}</p><button disabled={!selectedNode||!['repeater','list','carousel','gallery','table','container','section','grid','stack','flex'].includes(selectedNode.type)} onClick={repeatSelected}>Repeat children from collection</button>{collection?.fields?.map(field=><button disabled={!selectedNode} key={field.id} onClick={()=>bindSelected(field)}>Bind {field.name}</button>)}</>}
