...
export function CanvasNode({nodeId}:{nodeId:string}){
 ...
 return <Tag {...props}>{marquee&&node.type==='page'&&<span className="studio-marquee" style={marquee}/>} {isSelected&&!cropMode&&<div className="studio-floating-actions" aria-hidden="true">•••</div>}{isSelected&&!cropMode&&<button className="studio-rotate-handle" aria-label="Rotate selection" onPointerDown={startRotate}><span/></button>}{isSelected&&!cropMode&&['top-left','top','top-right','right','bottom-right','bottom','bottom-left','left'].map(renderHandle)}{content}{node.children.map(id=><CanvasNode key={id} nodeId={id}/>)}</Tag>;
}
