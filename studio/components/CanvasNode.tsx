...
export function CanvasNode({nodeId}:{nodeId:string}){
 ...
 React.useEffect(()=>()=>{cropCleanup.current?.();rotationCleanup.current?.()},[]);
 ...
 const endDrag=(rect:any)=>{setRectOverride(null);dispatch({type:'SET_SNAP_LINES',payload:[]});if(!state.document)return;const geometry={position:'absolute',left:`${Math.round(rect.x)}px`,top:`${Math.round(rect.y)}px`};dragBase.current=null;dispatch({type:'UPDATE_NODE_GEOMETRY',payload:{nodeId,geometry}})};
 ...
 const endResize=(rect:any)=>{setRectOverride(null);dispatch({type:'SET_SNAP_LINES',payload:[]});dispatch({type:'UPDATE_NODE_GEOMETRY',payload:{nodeId,geometry:{position:'absolute',width:`${Math.round(rect.w)}px`,height:`${Math.round(rect.h)}px`,left:`${Math.round(rect.x)}px`,top:`${Math.round(rect.y)}px`}}})};
 ...
 return <Tag {...props}>{marquee&&node.type==='page'&&<span className="studio-marquee" style={marquee}/>} {singleSelected&&<div className="studio-floating-actions" aria-hidden="true">•••</div>}{singleSelected&&<button className="studio-rotate-handle" aria-label="Rotate selection" onPointerDown={startRotate}><span/><em>{rotationDraft!==null?`${Math.round(rotationDraft)}°`:''}</em></button>}{singleSelected&&['top-left','top','top-right','right','bottom-right','bottom','bottom-left','left'].map(renderHandle)}{content}{node.children.map(id=><CanvasNode key={id} nodeId={id}/>)}</Tag>;
}
