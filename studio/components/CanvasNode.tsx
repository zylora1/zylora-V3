...
function EditableText({nodeId,value,runs,onChange,onEnter}:{nodeId:string;value:string;runs:any[];onChange:(value:string)=>void;onEnter:()=>void}){
 ...
 return <span {...richProps}
   ...
   onBlur={e=>{setEditing(false);if((window as any).__zyloraRichSelection?.nodeId===nodeId)delete (window as any).__zyloraRichSelection;onChange(e.currentTarget.textContent||'')}}/>;
}
...
export function CanvasNode({nodeId}:{nodeId:string}){
 ...
 React.useEffect(()=>()=>{cropCleanup.current?.();clearTransientSnapLines()},[]);
 ...
 const rotationDraft=rotation.angle;
