...
export class CLISessionImpl implements CLISession {
    ...
    private async ensureXTermLibraries(): Promise<boolean> {
        if (!FitAddonClass || !TerminalClass) {
            try {
                ...
                // In packaged client builds or sandbox preview pages, xterm is optional
                return false;
            }
        }
        return true;
    ...
    async initTerminal() {
        try {
            const ready = await this.ensureXTermLibraries();
            if (!ready || !FitAddonClass || !TerminalClass) {
                return;
            }
