...
export default function TemplateGallery(){
 ...
 useEffect(()=>{api<T[]>('/templates/public').then(setList).catch((e:any)=>setMsg(e.message))},[]);
 ...
 return <>
  ...
  <div className="templateGrid premiumTemplateGrid">{shown.map((t,i)=><article className={`templateCard family-${t.family}`} key={t.key}>
   <Link className="templatePreviewLink" href={`/template-preview/${encodeURIComponent(t.key)}`} target="_blank" aria-label={`Preview ${t.display_name}`}>
    <div className="templatePreview" style={t.preview_image?{backgroundImage:`url(${t.preview_image})`,backgroundSize:'cover',backgroundPosition:'center'}:undefined}><span>{t.featured?'Featured':String((page-1)*PAGE_SIZE+i+1).padStart(3,'0')}</span></div>
   </Link>
