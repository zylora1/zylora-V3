...
$$('.rail-btn[data-view]').forEach(b=>b.classList.toggle('active',b.dataset.view===id));if(id==='overview')renderHeroSiteCard();if(id==='assistant')loadDedicatedAssistant();if(id==='appointments')loadAppointments();if(id==='leads')loadLeads();if(id==='analytics')loadGrowth();if(id==='health')loadHealth();if(id==='settings')loadSettings();if(id==='billing')loadBilling();if(id==='domains')loadDomains();if(id==='integrations'){loadGoogleSheet();loadKnowledge()}if(id==='freelancer')loadFreelancer();if(id==='support')loadSupport();window.scrollTo({top:0,behavior:'smooth'})}
  ...
  for(const s of state.sites){
    ...
    c.innerHTML = `
      ...
      <div class="site-card-body">
        ...
        <div class="site-meta">
          <span>${escapeHtml(s.origin==='AI'?'Prompt-derived':s.origin==='IMPORT'?'Imported source':'Template: '+(s.template_slug||'Catalogue'))}</span>
        </div>
        <div class="site-actions restrained-actions">
          <a class="site-primary" href="/editor/${s.id}">Edit website</a>
...
async function useTemplateInDashboard(slug){
  try {
    toast('Creating website from template…');
    const tmpl = state.templates?.find(t => t.slug === slug) || {};
    const name = tmpl.name ? `${tmpl.name} Site` : 'My Website';
    const res = await api('/api/sites', {
      ...
      body: JSON.stringify({
        ...
        template_slug: slug,
        business_name: name,
        tagline: tmpl.description || 'Welcome to our website',
        style: 'swiss-minimal'
      })
    });
    if (res.id) {
      location.href = `/editor/${encodeURIComponent(res.id)}`;
    }
  } catch (err) {
    toast(err.message || 'Could not create site from template');
  }
}
function renderTemplates(){
  ...
  if(!state.templates.length){
    g.innerHTML=`<div class="dashboard-empty-template"><span>0 templates</span><h2>Catalogue review in progress.</h2><p>Only independently verified templates are published here.</p><a class="accent-btn" href="/ai-create">Create with AI</a></div>`;
    return;
  }
  g.innerHTML=state.templates.map(t=>`
    <article class="template-item">
      <div class="template-preview">
        <img src="${escapeHtml(t.preview||'')}" alt="Rendered preview of ${escapeHtml(t.name)}" loading="lazy" onerror="this.onerror=null;this.style.display='none';if(this.nextElementSibling)this.nextElementSibling.style.display='flex';">
        <div class="template-fallback-preview" style="display:${t.preview?'none':'flex'}"><span class="fallback-icon">❖</span><b>${escapeHtml(t.name)}</b></div>
      </div>
      <h3>${escapeHtml(t.name)}</h3>
      <p class="template-kicker">${escapeHtml(t.industry||t.category||'Website')} · ${Number(t.pages||1)} page${Number(t.pages||1)===1?'':'s'}</p>
      <p class="template-description">${escapeHtml(t.description||t.art_direction||'Purpose-built website template.')}</p>
      <div class="template-actions">
        <a href="/template-preview/${encodeURIComponent(t.slug)}" target="_blank" rel="noopener">Preview</a>
        <button type="button" data-template-use="${escapeHtml(t.slug)}">Use template</button>
      </div>
    </article>
  `).join('');
  $$('[data-template-use]').forEach(b=>b.onclick=()=>useTemplateInDashboard(b.dataset.templateUse));
}
...
async function renderHeroSiteCard(){
  ...
  if(!site){
    ...
    if(fallbackMessage)fallbackMessage.innerHTML='Your first website starts here.<br><small>Create with AI or choose a verified template.</small>';
  ...
  if($('#heroEditSiteBtn')){$('#heroEditSiteBtn').textContent='Edit Website';$('#heroEditSiteBtn').href=`/editor/${site.id}`;$('#heroEditSiteBtn').onclick=null;}
...
const DASHBOARD_VIEWS = [
  ...
  { id: 'templates', title: 'Templates Catalogue', subtitle: 'Curated responsive designs', icon: '🎨', badge: 'View' },
...
const DASHBOARD_ACTIONS = [
  { id: 'act-new-site', title: '＋ Create New Website', subtitle: 'Launch from template or custom brief', action: () => openCreate(), icon: '✨', badge: 'Action' },
  { id: 'act-ai-create', title: 'Build with AI Builder', subtitle: 'Derive full site structure from prompt', action: () => { location.href = '/ai-create'; }, icon: '🤖', badge: 'Action' },
...
function renderGlobalSearchResults(query) {
  ...
  const matchingSites = (state.sites || []).filter(s => 
    ...
    subtitle: `${s.status} · /s/${s.slug} · Template: ${s.template_slug || 'custom'}`,
    icon: '💻',
    badge: 'Website'
  }));
  ...
  const matchingTemplates = (state.templates || []).filter(t => 
    ...
    template: t,
    title: t.name || t.slug,
    subtitle: `${t.category || 'Template'} · ${t.description || ''}`.slice(0, 70),
    icon: '🎨',
    badge: 'Template'
  }));
  ...
  if (matchingTemplates.length > 0) {
    html += '<div class="search-group-title">Templates</div>';
...
function activateSearchResult(idx) {
  ...
  if (result.type === 'view') {
    ...
    location.href = `/editor/${result.site.id}`;
