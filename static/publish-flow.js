(()=>{
  ...
  function planCards(plans,onChoose){
    const body=overlay.querySelector('[data-zpf-body]');body.innerHTML=`<div class="zpf-grid">${(plans||[]).map(p=>`<article class="zpf-plan ${p.plan==='STARTER'?'recommended':''}"><span class="zpf-kicker">${p.plan==='STARTER'?'Best to start':'Self-service'}</span><h3>${esc(p.name||p.plan)}</h3><div class="zpf-price">${p.plan==='FREE'?'Free':p.plan==='STARTER'?'₹799 India · US$9 elsewhere':p.plan==='GROWTH'?'₹1,799 India · US$19 elsewhere':'Contact us'}</div><ul><li>${Number(p.page_limit||0)} template pages</li><li>${p.is_paid?'Keeps layout & sizing edits':'Text & images always kept'}</li>${p.plan==='FREE'?'<li>Layout/sizing reset on publish</li>':'<li>Structural edits publish as designed</li>'}</ul><button class="zpf-btn" data-zpf-plan="${esc(p.plan)}">Choose ${esc(p.name||p.plan)}</button></article>`).join('')}</div>`;body.querySelectorAll('[data-zpf-plan]').forEach(b=>b.onclick=()=>onChoose(b.dataset.zpfPlan));
  }
  ...
  window.ZyloraPublishFlow={start(siteId,{onPublished,onError}={}){return execute({siteId,onPublished,onError},{})},close};
})();
